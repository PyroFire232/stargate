...
function stargate.OnStargateBuilt(e,ev)
	...
	if(e.surface.count_entities_filtered{position=e.position,name="stargate",radius=32}>1)then
...
function stargate.BeltLogistics()
	for idx,gate in pairs(global.stargates)do if(gate.isopen and not gate.dialing)then local vgate=global.stargates[gate.isopen]
		...
		if(gate.dhd)then
			...
			elseif(not gate.isopen and not gate.dialing)then gate.dht=gate.dht or {}
				...
				if(gate.dht.rolllock and gate.dht.rolllock<game.tick and (gate.dht.input or 0)<game.tick)then gate.dht.rolllock=nil
					gate.ent.surface.play_sound{path="stargate_chevron",position=gate.ent.position,volume=0.25}
				end
						...
						if((gate.dht.input or 0)<game.tick)then
							...
							if(c==1)then
								...
								if(table_size(gate.dht.chevrons)>5)then
									gate.dht=nil
									gate.ent.surface.play_sound{path="stargate_dial_fail",position=gate.ent.position,volume=0.25}
								...
								elseif((gate.dht.input or 0)<game.tick and (gate.dht.rolling or 0)>game.tick)then
									...
									gate.dht.rolling=game.tick+(180+60) --3*60
									gate.dht.rolllock=game.tick+180
									gate.dht.input=game.tick+10
									gate.ent.surface.play_sound{path="stargate_gate_roll",position=gate.ent.position,volume=0.25}
								end
...
function guis.stargate_dial.click(ev)
	...
	if(menu and menu.gate)then
		menu.gate.ent.surface.play_sound{path="stargate_chevron_lock_dhd",menu.gate.ent.position}
		...
		local code=menu.dialcode if(code and #code==5)then
			...
			if(tgate)then
				...
				menu.gate.ent.surface.play_sound{path="stargate_dial_fail",position=menu.gate.ent.position}
			end
		end
	end
...
function guis.stargate_dial_buttons.click(ev)
	local ply=game.players[ev.player_index]
	local gate=global.players[ply.index]
	...
	if(not tgate or tgate.isopen)then
		game.print("Unable to connect stargates")
		return
	end
...
function stargate.UpdateMenuDialcode(ply,menu,code)
	...
	if(code)then
		menu.gate.ent.surface.play_sound{path="stargate_dhd",menu.ent.position}
	else
		menu.gate.ent.surface.play_sound{path="stargate_chevron",menu.ent.position}
	end
end
...
events.on_event(defines.events.on_entity_damaged,function(ev) if(ev.entity.name=="stargate")then local gate=stargate.GetGate(ev.entity)
...
function stargate.PopulateGrid(ply,menu,search) local gate=menu.gate local dcode=menu.dialcode
	...
	for k,n in StringPairs(gates)do local v=global.stargates[k] -- make clickable dial button with camera
		if(k~=gate.index)then has=true
			...
			btn.map=btn.minimap.add{type="minimap",surface_index=gate.ent.surface.index,zoom=2,force=v.ent.force.name,position=v.ent.position,ignored_by_interaction=true}
...
function stargate.OpenMenu(ply,e,gate)
	...
	if(not gate.isopen)then
		...
		menu.rename=fflow.add{name="stargate_rename",type="sprite-button",sprite="item/repair-pack",style="tool_button",visible=(e.force==ply.force)} --small_slot_button
		...
		local txt=fflow.stargate_shutdown or fflow.add{name="stargate_shutdown",type="button",style="red_back_button",
			caption="                                Shutdown                                "
		}
...
events.on_event(defines.events.on_player_changed_position,function(ev) local ply=game.players[ev.player_index] if(not ply.driving)then
	...
	local f=ply.surface
	local ents=f.find_entities_filtered{area=vector.square(ply.position,vector(2,2)),name="stargate"}
	for k,v in pairs(ents)do
		if(v.name=="stargate")then
...
function stargate.PreFinishOpenGate(gate,tgate,vtick)
	if(not gate.dialing or not tgate.dialing or gate.dialing~=vtick or tgate.dialing~=vtick)then return end
	gate.ent.surface.play_sound{path="stargate_open",position=gate.ent.position}
	tgate.ent.surface.play_sound{path="stargate_open",position=tgate.ent.position}
...
function stargate.LockChevron(gate,tgate,i,vtick)
	if(not gate.dialing or not tgate.dialing or gate.dialing~=vtick or tgate.dialing~=vtick)then return end
	local vi=(10-i)
	tgate.ent.surface.play_sound{path="stargate_chevrons_incoming",position=tgate.ent.position}
	gate.ent.surface.play_sound{path="stargate_chevrons_incoming",position=gate.ent.position}
...
function stargate.OpenGate(gate,tgate)
	...
	gate.ent.surface.play_sound{path="stargate_gate_roll",volume_modifier=0.5}
	tgate.ent.surface.play_sound{path="stargate_gate_roll",volume_modifier=0.5}

	script.on_nth_tick(vtick+10*25+90,function()
		stargate.FinishOpenGate(gate,tgate,vtick)
	end)

	script.on_nth_tick(vtick+10*25,function()
		stargate.PreFinishOpenGate(gate,tgate,vtick)
	end)


	for i=1,9,1 do local vi=(10-i)
		script.on_nth_tick(vtick+vi*25,function()
...
function stargate.LoadSprites(gate) local gtick=(global.tick or 0)
	if(gate.dialing)then
		...
		local xtick=gate.dialing+10*25+90
		if(xtick>gtick)then
			script.on_nth_tick(xtick,function()
				stargate.FinishOpenGate(gate,tgate,vtick)
			end)
		end
		local xtick=gate.dialing+10*25
		if(xtick>gtick)then
			script.on_nth_tick(xtick,function()
				stargate.PreFinishOpenGate(gate,tgate,vtick)
			end)
		...
		for i=1,9,1 do local vi=(10-i)
			local xtick=gate.dialing+vi*25
			if(xtick>gtick)then
			script.on_nth_tick(xtick,function()
				stargate.LockChevron(gate,tgate,i,vtick)
			end)
...
function stargate.Shutdown(gate)
	...
	if(gate.dialing)then
	tgate.ent.surface.play_sound{path="stargate_shutdown",position=tgate.ent.position}
	gate.ent.surface.play_sound{path="stargate_shutdown",position=gate.ent.position}
	else
